// OpenMP parallel matrix multiplication with B transposed. Combines the
// cache-friendly access pattern (transpose) with multi-threaded parallelism.

#include <omp.h>
#include "matrix_mul.h"

static void matmul_omp_transposed(const float *A, const float *Bt, float *C,
                                   int N) {
...
int main(int argc, char **argv) {
    ...
    for (int r = 0; r < num_runs; r++) {
        double start = get_time_sec();
        matmul_omp_transposed(A, Bt, C, N);
        double end = get_time_sec();
        times[r] = end - start;
    }
    print_stats("OpenMP MatMul (transposed B)", N, num_runs, times, cpu_best);
